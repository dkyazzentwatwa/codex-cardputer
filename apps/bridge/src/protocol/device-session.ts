...
export class DeviceSession {
  ...
  send(message: ServerMessage): void {
    if (this.socket.readyState === this.socket.OPEN) this.socket.send(encodeServerMessage(message));
  }
    ...
    try {
      ...
      if (!this.deviceId) {
        if (message.type !== "hello") {
          this.send({ type: "error", code: "HANDSHAKE_REQUIRED", message: "Send hello first", recoverable: false });
        ...
        this.send({ type: "task.snapshot", tasks: this.tasks.all().slice(0, 20) });
        this.send({ type: "macro.snapshot", macros: this.router.macroSnapshot() });
        for (const approval of this.approvals.list()) this.send({ type: "approval.open", approval });
        return;
      }
      if (message.type === "hello") return;
      for (const response of await this.router.handle(this.deviceId, message)) this.send(response);
    } catch (error) {
      const code = error instanceof Error && "code" in error ? String(error.code) : "INVALID_MESSAGE";
      this.send({
        type: "error",
        code: /^[A-Z][A-Z0-9_]+$/.test(code) ? code : "INVALID_MESSAGE",
        message: error instanceof Error ? error.message.slice(0, 160) : "Invalid message",
        recoverable: true,
      });
    }
  }
